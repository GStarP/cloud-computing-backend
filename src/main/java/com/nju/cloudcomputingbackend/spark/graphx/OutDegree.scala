...
object OutDegree {
  ...
  def run(): Unit = {
    ...
    val atResult = atGraph.outDegrees.map { case (id, degree) => (degree, universityHashCode(id.toString)) }
      ...
      .collect().mkString(",")
    val at_pw = new PrintWriter(new OutputStreamWriter(new FileOutputStream("/my/json/graphx/at_outDegree.json")), true)
    ...
    val followResult = followsGraph.outDegrees.map { case (id, degree) => (degree, universityHashCode(id.toString)) }
      ...
      .map { case (degree, name) => String.format(rankPattern, name, degree.toString) }
      .collect().mkString(",")
    val pw = new PrintWriter(new OutputStreamWriter(new FileOutputStream("/my/json/graphx/follows_outDegree.json")), true)
